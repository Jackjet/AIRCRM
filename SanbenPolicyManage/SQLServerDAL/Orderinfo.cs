...
namespace Sanben.SQLServerDAL
{
	...
	public partial class Orderinfo:IOrderinfo
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("pk_id", "Orderinfo"); 
		}
		...
		public int Add(Sanben.Model.Orderinfo model)
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@cabinnote", SqlDbType.VarChar,-1),
					...
					new SqlParameter("@policy", SqlDbType.Int,4)};
			...
			parameters[49].Value = model.policy;
		...
		public bool Update(Sanben.Model.Orderinfo model)
		{
			...
			parameters[48].Value = model.orderDesc;
			parameters[49].Value = model.policy;
			parameters[50].Value = model.pk_id;
		...
		/// 得到一个对象实体
		/// </summary>
		public Sanben.Model.Orderinfo DataRowToModel(DataRow row)
		{
			Sanben.Model.Orderinfo model=new Sanben.Model.Orderinfo();
			if (row != null)
			{
				...
				if(row["policy"]!=null && row["policy"].ToString()!="")
				{
					model.policy=int.Parse(row["policy"].ToString());
				}
			}
			return model;
		}
