...
{
	/// <summary>
	/// 数据访问类:kuXunPolicyOptions
	/// </summary>
	public partial class kuXunPolicyOptions:IkuXunPolicyOptions
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "kuXunPolicyOptions"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from kuXunPolicyOptions");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Sanben.Model.kuXunPolicyOptions model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into kuXunPolicyOptions(");
			strSql.Append("tolerate_stationary_point,tolerate_leave_money,lowest_rebate,highest_rebate,earliest_advance_days,night_before_days,rebate_class_set,discount,agent_rate,insurance_number,foundPnr,is_mail_itinerary,validatePatA,validateClass,refund_change_rule,sys_id,platform_type_id)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@agent_rate", SqlDbType.Float,8),
					new SqlParameter("@insurance_number", SqlDbType.Int,4),
					new SqlParameter("@foundPnr", SqlDbType.Bit,1),
					new SqlParameter("@is_mail_itinerary", SqlDbType.Bit,1),
					new SqlParameter("@validatePatA", SqlDbType.VarChar,50),
					new SqlParameter("@validateClass", SqlDbType.Int,4),
					new SqlParameter("@refund_change_rule", SqlDbType.NChar,255),
			...
			parameters[8].Value = model.agent_rate;
			parameters[9].Value = model.insurance_number;
			parameters[10].Value = model.foundPnr;
			parameters[11].Value = model.is_mail_itinerary;
			parameters[12].Value = model.validatePatA;
			parameters[13].Value = model.validateClass;
			parameters[14].Value = model.refund_change_rule;
			parameters[15].Value = model.sys_id;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Sanben.Model.kuXunPolicyOptions model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update kuXunPolicyOptions set ");
			...
			strSql.Append("agent_rate=@agent_rate,");
			strSql.Append("insurance_number=@insurance_number,");
			strSql.Append("foundPnr=@foundPnr,");
			strSql.Append("is_mail_itinerary=@is_mail_itinerary,");
			strSql.Append("validatePatA=@validatePatA,");
			strSql.Append("validateClass=@validateClass,");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from kuXunPolicyOptions ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Sanben.Model.kuXunPolicyOptions GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,tolerate_stationary_point,tolerate_leave_money,lowest_rebate,highest_rebate,earliest_advance_days,night_before_days,rebate_class_set,discount,agent_rate,insurance_number,foundPnr,is_mail_itinerary,validatePatA,validateClass,refund_change_rule,sys_id,platform_type_id from kuXunPolicyOptions ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Sanben.Model.kuXunPolicyOptions DataRowToModel(DataRow row)
		{
			Sanben.Model.kuXunPolicyOptions model=new Sanben.Model.kuXunPolicyOptions();
			if (row != null)
			{
				...
				if(row["agent_rate"]!=null && row["agent_rate"].ToString()!="")
				{
					model.agent_rate=decimal.Parse(row["agent_rate"].ToString());
				}
				if(row["insurance_number"]!=null && row["insurance_number"].ToString()!="")
				{
					model.insurance_number=int.Parse(row["insurance_number"].ToString());
				}
				if(row["foundPnr"]!=null && row["foundPnr"].ToString()!="")
				{
					if((row["foundPnr"].ToString()=="1")||(row["foundPnr"].ToString().ToLower()=="true"))
					{
						model.foundPnr=true;
					}
					else
					{
						model.foundPnr=false;
					}
				}
				...
				if(row["validatePatA"]!=null)
				{
					model.validatePatA=row["validatePatA"].ToString();
				}
				if(row["validateClass"]!=null && row["validateClass"].ToString()!="")
				{
					model.validateClass=int.Parse(row["validateClass"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,tolerate_stationary_point,tolerate_leave_money,lowest_rebate,highest_rebate,earliest_advance_days,night_before_days,rebate_class_set,discount,agent_rate,insurance_number,foundPnr,is_mail_itinerary,validatePatA,validateClass,refund_change_rule,sys_id,platform_type_id ");
			strSql.Append(" FROM kuXunPolicyOptions ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" id,tolerate_stationary_point,tolerate_leave_money,lowest_rebate,highest_rebate,earliest_advance_days,night_before_days,rebate_class_set,discount,agent_rate,insurance_number,foundPnr,is_mail_itinerary,validatePatA,validateClass,refund_change_rule,sys_id,platform_type_id ");
			strSql.Append(" FROM kuXunPolicyOptions ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from kuXunPolicyOptions T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "kuXunPolicyOptions";
