...
namespace RunTask
{
    class run
    {
        ...
        #endregion
        public run(int sysid,bool isAll)
        {
            _sysid = sysid;
            _isAll = isAll;
            Init();
        ...
        public void BuildXmlAndGzip()
        {
            ...
            {
                string cabin = ds.Tables[0].Rows[i]["cabin"].ToString().Replace('/', ',');
                DataRow dr = tmpds.Tables["Policy"].NewRow();
                tmpds.Tables["Policy"].Rows.Add(dr);
                tmpds.Tables["Policy"].Rows[i]["flightcode"] = ds.Tables[0].Rows[i]["Company"];//航空公司
                tmpds.Tables["Policy"].Rows[i]["policyCode"] = username + ds.Tables[0].Rows[i]["Company"] + "_" + "JINRI" + "_" + ds.Tables[0].Rows[i]["PolicyId"];//政策标识
                tmpds.Tables["Policy"].Rows[i]["dpt"] = ds.Tables[0].Rows[i]["ScityE"];//出发地
                tmpds.Tables["Policy"].Rows[i]["arr"] = ds.Tables[0].Rows[i]["EcityE"];//到达地
                tmpds.Tables["Policy"].Rows[i]["flightNumLimit"] = "所有";//航班限制 === 所有、适用、不适用 
                tmpds.Tables["Policy"].Rows[i]["flightcondition"] = "";//航班号
                tmpds.Tables["Policy"].Rows[i]["cabin"] = cabin.Substring(0, cabin.Length - 1); //只有普通政策才能指定多仓位
                tmpds.Tables["Policy"].Rows[i]["discountType"] = "指定票面价";//票面价类型
                tmpds.Tables["Policy"].Rows[i]["discountValue"] = 0;//票面价/折扣
                tmpds.Tables["Policy"].Rows[i]["returnpoint"] = ds.Tables[0].Rows[i]["Discounts"];//返点
                ...
                tmpds.Tables["Policy"].Rows[i]["startdate"] = ds.Tables[0].Rows[i]["Sdate"];//旅行开始日期
                tmpds.Tables["Policy"].Rows[i]["enddate"] = ds.Tables[0].Rows[i]["Edate"];//旅行截止日期
                ...
                tmpds.Tables["Policy"].Rows[i]["officeno"] = ds.Tables[0].Rows[i]["OfficeNum"];//支持代码共享航班
            ...
            #region 生成XML文件-UTF8
            string strFileName = Application.StartupPath + @"\jinri.xml";
