...
namespace RunTask
{
    class BQY
    {
        ...
        #endregion
        public BQY(int sysid,bool isAll)
        {
            _sysid = sysid;
            _isAll = isAll;
            Init();
        ...
        {
            //string strWhere = @" airline in (select navigation_company from Policy_filter ) ";
            //strWhere += string.Format(@" and point between {0} and {1}", lRabete, hRabete);
            //ds = _bqyPolicyBll.GetList(strWhere);

            string strWhere = string.Format(@" airline in (	select navigation_company from Policy_filter)   
                                                       AND startcity IN (
                                                           SELECT StartCode FROM AirMan
                                                           WHERE CarrierName in (	select navigation_company from Policy_filter))");
        ...
        public void DiscountsModefy()
        {
            ...
            {
                object st1 = ds.Tables[0].Rows[i]["airline"].ToString(); //
                string scity = ds.Tables[0].Rows[i]["startcity"].ToString();//
                string ecity = ds.Tables[0].Rows[i]["arrivecity"].ToString();//
                object st3 = ds.Tables[0].Rows[i]["point"].ToString(); //政策源返点
                //自定义留点
                if (rebateCount > 0)
                {
        ...
        public void GeiQian()
        {
            ...
            for (int i = 0; i < counts; i++)
            {
                string st1 = ds.Tables[0].Rows[i]["airline"].ToString();
        ...
        public void BuildXmlAndGzip()
        {
            ...
            {
                lines = ds.Tables[0].Rows[i]["startcity"].ToString();//航线组
                endLines = ds.Tables[0].Rows[i]["arrivecity"].ToString();//
                string company = ds.Tables[0].Rows[i]["airline"].ToString();//航空公司
                if (!string.IsNullOrEmpty(lines))
                {
                    if (lines.Length > 3)//如果出发地有多个
                    {
                        ...
                        tmpstr = ds.Tables[0].Rows[i]["cabin_discount"].ToString().Substring(1);//去掉第一个字符‘#’

                        if (tmpstr[tmpstr.Length - 1] == '/')
                        {
                            tmpstr = ds.Tables[0].Rows[i]["cabin_discount"].ToString().Substring(1);//去掉最后一个字符‘/’
                        ...
                        for (int j = 0; j < lines.Length; j++)//每个出发地添加一条记录
                        {
                            ...
                            #region Cells Data
                            tmpds.Tables["Policy"].Rows[j]["flightcode"] = ds.Tables[0].Rows[i]["airline"];//航空公司
                            tmpds.Tables["Policy"].Rows[j]["policyCode"] = username + ds.Tables[0].Rows[i]["airline"] + "_" + "8000yi" + "_" + ds.Tables[0].Rows[i]["policy_num"];//政策标识
                            tmpds.Tables["Policy"].Rows[j]["dpt"] = "";//出发地
                            tmpds.Tables["Policy"].Rows[j]["arr"] = "";//到达地
                            tmpds.Tables["Policy"].Rows[j]["flightNumLimit"] = "所有";//航班限制 === 所有、适用、不适用 
                            tmpds.Tables["Policy"].Rows[j]["flightcondition"] = "";//航班号
                            ...
                            tmpds.Tables["Policy"].Rows[j]["returnpoint"] = ds.Tables[0].Rows[i]["point"];//返点
                            ...
                            tmpds.Tables["Policy"].Rows[j]["startdate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["startdate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行开始日期
                            tmpds.Tables["Policy"].Rows[j]["enddate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["arrivedate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行截止日期
                    ...
                    else if (lines.Length == 3)//如果出发地只有一个
                    {
                        ...
                        DataRow dr = tmpds.Tables["Policy"].NewRow();
                        #region Cells Data
                        tmpds.Tables["Policy"].Rows[i]["flightcode"] = ds.Tables[0].Rows[i]["airline"];//航空公司
                        tmpds.Tables["Policy"].Rows[i]["policyCode"] = username + ds.Tables[0].Rows[i]["airline"] + "_" + "8000yi" + "_" + ds.Tables[0].Rows[i]["policy_num"];//政策标识
                        tmpds.Tables["Policy"].Rows[i]["dpt"] = lines;//出发地
                        tmpds.Tables["Policy"].Rows[i]["arr"] = "PEK";//到达地
                        tmpds.Tables["Policy"].Rows[i]["flightNumLimit"] = "所有";//航班限制 === 所有、适用、不适用 
                        tmpds.Tables["Policy"].Rows[i]["flightcondition"] = "";//航班号
                        ...
                        tmpds.Tables["Policy"].Rows[i]["returnpoint"] = ds.Tables[0].Rows[i]["point"];//返点
                        ...
                        tmpds.Tables["Policy"].Rows[i]["startdate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["startdate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行开始日期
                        tmpds.Tables["Policy"].Rows[i]["enddate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["arrivedate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行截止日期
                ...
                //            #region Cells Data
                //            tmpds.Tables["Policy"].Rows[j]["flightcode"] = ds.Tables[0].Rows[i]["airline"];//航空公司
                //            tmpds.Tables["Policy"].Rows[j]["policyCode"] = ds.Tables[0].Rows[i]["airline"] + "_" + "8000yi" + "_" + username + ds.Tables[0].Rows[i]["policy_num"];//政策标识
                //            tmpds.Tables["Policy"].Rows[j]["dpt"] = "";//出发地
                //            tmpds.Tables["Policy"].Rows[j]["arr"] = "";//到达地
                //            tmpds.Tables["Policy"].Rows[j]["flightNumLimit"] = "所有";//航班限制 === 所有、适用、不适用 
                //            tmpds.Tables["Policy"].Rows[j]["flightcondition"] = "";//航班号
                ...
                //            tmpds.Tables["Policy"].Rows[j]["discountValue"] = 0;//票面价/折扣
                //            tmpds.Tables["Policy"].Rows[j]["returnpoint"] = ds.Tables[0].Rows[i]["point"];//返点
                //            tmpds.Tables["Policy"].Rows[j]["returnprice"] = 0;//留钱
                //            tmpds.Tables["Policy"].Rows[j]["startdate_ticket"] = DateTime.Now.ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//政策有效期开始日期
                //            tmpds.Tables["Policy"].Rows[j]["enfdate_ticket"] = DateTime.Now.AddDays(7.0).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//政策有效期截止日期
                //            tmpds.Tables["Policy"].Rows[j]["startdate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["startdate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行开始日期
                //            tmpds.Tables["Policy"].Rows[j]["enddate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["arrivedate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行截止日期
                //            tmpds.Tables["Policy"].Rows[j]["beforeValidDay"] = "0";//提前出票时限
                //            tmpds.Tables["Policy"].Rows[j]["backnote"] = "";//退改签说明
                //            tmpds.Tables["Policy"].Rows[j]["cabinnote"] = ds.Tables[0].Rows[i]["cabinDescription"];//仓位说明
                //            tmpds.Tables["Policy"].Rows[j]["autoTicket"] = "否";//退改签说明
                //            tmpds.Tables["Policy"].Rows[j]["shared"] = "否";//支持代码共享航班
                //            tmpds.Tables["Policy"].Rows[j]["officeno"] = ds.Tables[0].Rows[i]["office_code"];//支持代码共享航班
                //            tmpds.Tables["Policy"].Rows[j]["xcd"] = 1;//行程单
                //            tmpds.Tables["Policy"].Rows[j]["flyerpoints"] = configSet.Tables[0].Rows[0]["is_provide_regular_passenger_integral"].ToString() == "True" ? "是" : "否";//是否提供常旅客积分
                //            tmpds.Tables["Policy"].Rows[j]["cardType"] = configSet.Tables[0].Rows[0]["document_type"].ToString();//证件类型  
                //            tmpds.Tables["Policy"].Rows.Add(dr);
                ...
                //        #region Cells Data
                //        tmpds.Tables["Policy"].Rows[i]["flightcode"] = ds.Tables[0].Rows[i]["airline"];//航空公司
                //        tmpds.Tables["Policy"].Rows[i]["policyCode"] = ds.Tables[0].Rows[i]["airline"] + "_" + "8000yi" + "_" + username + ds.Tables[0].Rows[i]["policy_num"];//政策标识
                //        tmpds.Tables["Policy"].Rows[i]["dpt"] = lines;//出发地
                //        tmpds.Tables["Policy"].Rows[i]["arr"] = "PEK";//到达地
                //        tmpds.Tables["Policy"].Rows[i]["flightNumLimit"] = "所有";//航班限制 === 所有、适用、不适用 
                //        tmpds.Tables["Policy"].Rows[i]["flightcondition"] = "";//航班号
                ...
                //        tmpds.Tables["Policy"].Rows[i]["discountValue"] = 0;//票面价/折扣
                //        tmpds.Tables["Policy"].Rows[i]["returnpoint"] = ds.Tables[0].Rows[i]["point"];//返点
                //        tmpds.Tables["Policy"].Rows[i]["returnprice"] = 0;//留钱
                //        tmpds.Tables["Policy"].Rows[i]["startdate_ticket"] = DateTime.Now.ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//政策有效期开始日期
                //        tmpds.Tables["Policy"].Rows[i]["enfdate_ticket"] = DateTime.Now.AddDays(7.0).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//政策有效期截止日期
                //        tmpds.Tables["Policy"].Rows[i]["startdate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["startdate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行开始日期
                //        tmpds.Tables["Policy"].Rows[i]["enddate"] = Convert.ToDateTime(ds.Tables[0].Rows[i]["arrivedate"]).ToString("yyyy-MM-dd", DateTimeFormatInfo.InvariantInfo);//旅行截止日期
                //        tmpds.Tables["Policy"].Rows[i]["beforeValidDay"] = "0";//提前出票时限
                //        tmpds.Tables["Policy"].Rows[i]["backnote"] = "";//退改签说明
                //        tmpds.Tables["Policy"].Rows[i]["cabinnote"] = ds.Tables[0].Rows[i]["cabinDescription"];//仓位说明
                //        tmpds.Tables["Policy"].Rows[i]["autoTicket"] = "否";//退改签说明
                //        tmpds.Tables["Policy"].Rows[i]["shared"] = "否";//支持代码共享航班
                //        tmpds.Tables["Policy"].Rows[i]["officeno"] = ds.Tables[0].Rows[i]["office_code"];//支持代码共享航班
                //        tmpds.Tables["Policy"].Rows[i]["xcd"] = 1;//行程单
                //        tmpds.Tables["Policy"].Rows[i]["flyerpoints"] = configSet.Tables[0].Rows[0]["is_provide_regular_passenger_integral"].ToString() == "True" ? "是" : "否";//是否提供常旅客积分
                //        tmpds.Tables["Policy"].Rows[i]["cardType"] = configSet.Tables[0].Rows[0]["document_type"].ToString();//证件类型  
                //        tmpds.Tables["Policy"].Rows.Add(dr);
            ...
            #region 生成XML文件-UTF8
            string strFileName = Application.StartupPath + @"\8000yi.xml";
